...
function DarthVaderIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden="true">
      {/* base oscura del casco — casi invisible sobre el fondo oscuro del panel */}
...
const SERVICE_ICONS = [
  <Code2 key="code" className="size-7 text-accent" />,
  <LayoutDashboard key="dash" className="size-7 text-accent" />,
  <Globe key="globe" className="size-7 text-accent" />,
  <DarthVaderIcon key="vader" className="size-7 text-accent" />,
];
        ...
        <div className="grid grid-cols-2 gap-3">
          {services.map((service, index) => (
            <MotionFade
              key={service}
              delay={index * 0.08}
              className="flex flex-col items-start gap-2.5 rounded-2xl border border-line bg-panel px-4 py-4 shadow-[0_20px_56px_rgba(0,0,0,0.22)] backdrop-blur-[14px]"
            >
              <div className="rounded-xl bg-accent/10 p-2">
                {SERVICE_ICONS[index]}
              </div>
              <span className="font-bold text-accent text-sm leading-snug">{service}</span>
            </MotionFade>
          ))}
        </div>
      </div>
