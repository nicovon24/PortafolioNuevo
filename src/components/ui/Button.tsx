...
export function IconButton({ label, className, children, ...rest }: IconButtonProps) {
  return (
    <button
      ...
      {...rest}
      className={cn(
        "control-surface grid shrink-0 place-items-center rounded-full border text-accent transition-colors hover:border-accent-2 hover:text-accent-2",
        focusRing,
        className,
      )}
    >
...
export function TechChip({ children }: { children: ReactNode }) {
  return (
    <span className="control-surface rounded-full border px-2.5 py-1 font-mono text-mini font-semibold text-ink/90 transition-colors hover:border-accent/40 hover:text-accent">
      {children}
    </span>
  );
}
